...
Jimp.read(posterImageName).then(function(image) {
    ...
    Jimp.read(startImageName).then(function (image) {
        ...
        while (generation <= endGeneration) {
            ...
            let newImage = new Jimp(width, height, function (err, newImage) {
                // Update new image with new version that has swapped pixels
                for (let x = 0; x < width; x++) {
                    for (let y = 0; y < height; y++) {
                        let n = img[x][y];
                        let p = pix[n];
                        ...
                        newImage.setPixelColor(hex, x, y);
            ...
            // DEBUG: Statistics: Average distance of pixels to centers
